...
const benefits = [
  {
    ...
    body: "Creative preparation and production coordinated in one workflow.",
    icon: "design",
    accent: "cyan",
  },
  {
    title: "Multi-Surface Consistency",
    body: "Graphics matched across display systems and applications.",
    icon: "workflow",
    accent: "magenta",
  },
  {
    title: "Fast Campaign Execution",
    body: "Production planning suited to event and launch timelines.",
    icon: "scale",
    accent: "yellow",
  },
  {
    title: "UAE-Focused Delivery",
    body: "Visual production and installation support across the UAE.",
    icon: "pin",
    accent: "cyan",
  },
] as const;
        ...
        <div className="grid gap-8 lg:grid-cols-12">
          <Reveal from="up" className="lg:col-span-7">
            ...
            <h2 className="display-md mt-6 overflow-visible text-balance pt-[0.06em] pb-[0.14em] pl-[0.03em] text-white">
              ...
              <br />
              Every Physical{" "}
              <span className="text-gradient-hero">Touchpoint.</span>
            </h2>
          </Reveal>
        </div>
            ...
            return (
              <Reveal key={item.title} from="up" delay={i * 0.1}>
                <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition-all duration-300 hover:-translate-y-1.5 hover:border-white/25 sm:p-7">
                  ...
                  >
                    <ReIcon name={item.icon} className="size-9" />
                  </span>

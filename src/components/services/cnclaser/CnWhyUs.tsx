...
const benefits = [
  {
    ...
    body: "Digital preparation and physical production coordinated together.",
    icon: "prepare",
    accent: "cyan",
  },
  {
    title: "Application-Led Materials",
    body: "Material and process selected around the final use.",
    icon: "material",
    accent: "magenta",
  },
  {
    title: "Consistent Repeatability",
    body: "Approved files reused for repeat production where appropriate.",
    icon: "repeat",
    accent: "yellow",
  },
  {
    title: "UAE-Focused Execution",
    body: "Custom fabrication support for projects across the UAE.",
    icon: "pin",
    accent: "cyan",
  },
] as const;
        ...
        <div className="grid gap-8 lg:grid-cols-12">
          <Reveal from="up" className="lg:col-span-7">
            ...
            <h2 className="display-md mt-6 overflow-visible text-balance pt-[0.06em] pb-[0.14em] pl-[0.03em] text-white">
              ...
              <br />
              Of The Production{" "}
              <span className="text-gradient-hero">Standard.</span>
            </h2>
          </Reveal>
        </div>
            ...
            return (
              <Reveal key={item.title} from="up" delay={i * 0.1}>
                <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition-all duration-300 hover:-translate-y-1.5 hover:border-white/25 sm:p-7">
                  ...
                  >
                    <CnIcon name={item.icon} className="size-9" />
                  </span>
